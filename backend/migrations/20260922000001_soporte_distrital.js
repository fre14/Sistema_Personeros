...
export async function up(knex) {
  ...
  await knex.schema.alterTable(TABLE_CANDIDATOS, (table) => {
    ...
    table.integer('distrito_id').unsigned().references('id').inTable(TABLE_DISTRITOS);
  ...
  await knex.schema.raw(`
    CREATE UNIQUE INDEX idx_resultado_mesa_tipo_activo
    ON ${TABLE_RESULTADOS}(mesa_id, tipo_eleccion)
  `);

  await knex.schema.raw(`CREATE INDEX idx_candidatos_tipo ON ${TABLE_CANDIDATOS}(tipo_eleccion, activo)`);
  await knex.schema.raw(`CREATE INDEX idx_candidatos_distrito ON ${TABLE_CANDIDATOS}(distrito_id) WHERE distrito_id IS NOT NULL`);
  await knex.schema.raw(`CREATE INDEX idx_resultados_tipo ON ${TABLE_RESULTADOS}(tipo_eleccion)`);
  await knex.schema.raw(`CREATE INDEX idx_resultados_mesa_tipo ON ${TABLE_RESULTADOS}(mesa_id, tipo_eleccion)`);
  await knex.schema.raw(`CREATE INDEX idx_mesas_estado_distrital ON ${TABLE_MESAS}(estado_distrital) WHERE estado_distrital IS NOT NULL`);
}
