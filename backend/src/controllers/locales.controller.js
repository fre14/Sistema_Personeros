...
export const getAll = async (req, res) => {
  try {
    ...
    let query = db('locales_votacion as locales')
...
export const getById = async (req, res) => {
  try {
    const { id } = req.params;
    const local = await db('locales_votacion as locales')
      .join('distritos', 'locales.distrito_id', 'distritos.id')
      .select('locales.*', 'distritos.nombre as distrito_nombre')
    ...
    const mesas_count = await db('mesas_sufragio as mesas').where({ local_id: id }).count('id as c').first();
...
export const create = async (req, res) => {
  try {
    const [id] = await db('locales_votacion').insert(req.body).returning('id');
    const local = await db('locales_votacion as locales').where({ id: id.id || id }).first();
...
export const update = async (req, res) => {
  try {
    const { id } = req.params;
    const oldData = await db('locales_votacion as locales').where({ id }).first();
    if (!oldData) return res.status(404).json({ success: false, message: 'Local no encontrado' });
    
    await db('locales_votacion').where({ id }).update(req.body);
    const local = await db('locales_votacion as locales').where({ id }).first();
...
export const remove = async (req, res) => {
  try {
    ...
    const count = await db('mesas_sufragio as mesas').where({ local_id: id }).count('id as c').first();
    if (parseInt(count.c) > 0) {
      return res.status(400).json({ success: false, message: 'Local tiene mesas asociadas' });
    }

    await db('locales_votacion').where({ id }).del();
